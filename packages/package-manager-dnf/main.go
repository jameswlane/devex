...
func (p *DnfPlugin) handleInstall(args []string) error {
	if len(args) == 0 {
	...
	return sdk.ExecCommand(true, "dnf", cmdArgs...)
}

func (p *DnfPlugin) handleRemove(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("no packages specified")
	}
...
func (p *DnfPlugin) handleUpdate(args []string) error {
	fmt.Println("Updating package repositories...")
	return sdk.ExecCommand(true, "dnf", "update")
}

func (p *DnfPlugin) handleSearch(args []string) error {
	if len(args) == 0 {
	...
	return sdk.ExecCommand(false, "dnf", "search", searchTerm)
}

func (p *DnfPlugin) handleList(args []string) error {
	return sdk.ExecCommand(false, "dnf", "list")
}
