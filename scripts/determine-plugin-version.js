...
class PluginVersionManager {
	...
	updatePackageVersion(pluginPath, newVersion) {
		...
		try {
			...
			console.error(
				`Error updating package.json for ${pluginPath}:`,
				error.message,
			);
			return false;
		}
	...
	getLastPluginTag(pluginName) {
		try {
			...
			const tags = execSync(
				`git tag -l "${pluginName}-v*" --sort=-version:refname`,
				{ encoding: "utf-8" },
			)
				.trim()
				.split("\n")
	...
	async processPlugin(pluginName, options = {}) {
		...
		if (commits.length === 0 && !options.force) {
			...
			if (!lastTag) {
				...
				const newVersion =
					currentVersion === "0.0.0" ? "1.0.0" : currentVersion;
		...
		const newVersion =
			options.version || this.incrementVersion(currentVersion, versionBump);
	...
	(async () => {
		switch (command) {
			case "check": {
				...
				if (!pluginName) {
					console.error(
						"Usage: determine-plugin-version.js check <plugin-name>",
					);
					process.exit(1);
				}
			...
			case "update": {
				...
				if (!pluginName) {
					console.error(
						"Usage: determine-plugin-version.js update <plugin-name>",
					);
					process.exit(1);
				}
				...
				if (result) {
					console.log(
						`Updated ${pluginName} from ${result.currentVersion} to ${result.newVersion}`,
					);
			...
			case "update-all": {
				...
				for (const plugin of plugins) {
					...
					if (result) {
						results.push(result);
						console.log(
							`Updated ${plugin}: ${result.currentVersion} → ${result.newVersion} (${result.bump})`,
						);
					}
				}
				break;
