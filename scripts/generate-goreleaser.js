...
class GoReleaserGenerator {
    ...
    async generateConfig() {
        ...
        await fs.writeFile(this.outputPath, yaml.dump(config, { 
            lineWidth: -1,
            noRefs: true,
            quotingType: '"',
            forceQuotes: false
        ...
        console.log(`✅ Generated GoReleaser config with ${plugins.length} plugins`);
    ...
    determinePlatforms(pluginName, packageJson = {}) {
        ...
        if (pluginName.includes('package-manager-')) {
            ...
            if (pluginName.includes('apk') || pluginName.includes('emerge') || 
                pluginName.includes('pacman') || pluginName.includes('dnf') ||
                pluginName.includes('rpm') || pluginName.includes('zypper') ||
                pluginName.includes('snap') || pluginName.includes('flatpak') ||
                pluginName.includes('appimage') || pluginName.includes('xbps') ||
                pluginName.includes('yay') || pluginName.includes('eopkg')) {
                return ['linux'];
            }
            ...
            brews: [
                {
                    ...
                    description: 'CLI tool for efficiently setting up and managing development environments',
                    license: 'GPL-3.0'
                }
            ],
            snapcrafts: [
                {
                    ...
                    description: 'DevEx is a powerful CLI tool designed to streamline the setup and management of development environments. It simplifies the installation of applications, configuration of programming languages, and customization of themes.',
                    grade: 'stable',
                    confinement: 'classic',
                    license: 'GPL-3.0'
                }
...
async function main() {
    try {
        ...
        console.error('❌ Failed to generate GoReleaser configuration:', error.message);
        process.exit(1);
    }
}
