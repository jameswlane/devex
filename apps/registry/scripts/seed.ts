...
async function getGitHubPath(type: 'application' | 'plugin', name: string): Promise<string> {
  if (type === 'plugin') {
...
async function seedApplications() {
  ...
  try {
    ...
    const webToolsPath = path.join(process.cwd(), '../web/app/generated/tools.json');
    const webToolsData: WebToolsData = JSON.parse(fs.readFileSync(webToolsPath, 'utf-8'));
    
    const applications = webToolsData.tools.filter(tool => tool.type === 'application');
    ...
    for (const app of applications) {
      ...
      const linuxSupport = app.platforms.linux ? await createPlatformInfo(app.platforms.linux) : null;
      const macosSupport = app.platforms.macos ? await createPlatformInfo(app.platforms.macos) : null;
      const windowsSupport = app.platforms.windows ? await createPlatformInfo(app.platforms.windows) : null;
...
async function seedPlugins() {
  ...
  try {
    ...
    const registryData: RegistryData = JSON.parse(fs.readFileSync(registryPath, 'utf-8'));
    ...
    for (const plugin of plugins) {
      ...
      if (plugin.tags.includes('linux')) {
        ...
        if (plugin.tags.includes('debian') || plugin.tags.includes('ubuntu') || plugin.tags.includes('apt')) {
          platforms = ['linux'];
        } else if (plugin.tags.includes('macos') || plugin.tags.includes('brew')) {
          platforms = ['macos'];
...
async function updateRegistryStats() {
  ...
  try {
    const [applicationsCount, pluginsCount, configsCount, stacksCount] = await Promise.all([
