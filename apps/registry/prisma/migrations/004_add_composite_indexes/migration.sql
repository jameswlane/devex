...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_category_platforms" 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_official_default_name" 
ON "applications"("official" DESC, "default" DESC, "name" ASC);

-- 3. Category + official for filtered browsing
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_category_official" 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_stacks_category_downloads" 
ON "stacks"("category", "downloadCount" DESC) 
WHERE "downloadCount" > 0;

-- Cross-table popularity queries
-- 10. Download tracking indexes for all resources
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_plugins_downloads_recent" 
ON "plugins"("downloadCount" DESC, "lastDownload" DESC NULLS LAST) 
WHERE "downloadCount" > 0;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_configs_downloads_recent" 
ON "configs"("downloadCount" DESC, "lastDownload" DESC NULLS LAST) 
WHERE "downloadCount" > 0;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_stacks_downloads_recent" 
ON "stacks"("downloadCount" DESC, "lastDownload" DESC NULLS LAST) 
WHERE "downloadCount" > 0;

-- Search optimization indexes
-- 11. Applications search optimization (name + description)
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_search_name" 
ON "applications" USING gin(to_tsvector('english', "name"));

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_search_desc" 
ON "applications" USING gin(to_tsvector('english', "description"));

-- 12. Plugins search optimization  
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_plugins_search_name" 
ON "plugins" USING gin(to_tsvector('english', "name"));

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_plugins_search_desc" 
ON "plugins" USING gin(to_tsvector('english', "description"));

-- Platform filtering optimization for JSON fields
-- 13. GIN indexes for platform JSON path queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_platforms_linux" 
ON "applications" USING gin(("platforms"->>'linux')) 
WHERE ("platforms"->>'linux') IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_platforms_macos" 
ON "applications" USING gin(("platforms"->>'macos')) 
WHERE ("platforms"->>'macos') IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_applications_platforms_windows" 
ON "applications" USING gin(("platforms"->>'windows')) 
